...
export class AppComponent {
  ...
  constructor(
    ...
    this.roleService.getRoleId((roles:any)=>{
      console.log("Roles",roles)
    })
    this.auth.isMemberOf({user_id:this.user.id,role_id:this.roles.plafoncreator},(res:any) => {
      if(res.length>0){
    ...
    this.auth.isMemberOf({user_id:this.user.id,role_id:this.roles.purchase},(res:any) => {
      if(res.length>0){
    ...
    this.auth.isMemberOf({user_id:this.user.id,role_id:this.roles.admin},(res:any) => {
      if(res.length>0){
        console.log("This user is a plafon creator")
        this.notadmin = false
      }else{
    ...
    this.auth.isMemberOfDivision({user_id:this.user.id,division_id:1},(res:any) => {
      if(res[0].cnt>0){
