...
//Variables for paginator
  ...
  _roles = {
    ...
    this.roleService.getRoleMails({identifier:'verifier'},(role:any)=>{
    ...
    this.statuses = this.commonService.statuses
      ...
      this.userService.getRoles({user_id:this.logindata.id},(roles:any) => {
        console.log("User Roles",roles)
        this.userRoles = roles.map((role:any)=>{
          return role.role_id
        }).join(",")
        this.submissionService.getSubmissionByRole({params:this.userRoles,user_id:this.logindata.id},(allSubmissions:any)=>{
      ...
      this.auth.isMemberOf({user_id:this.logindata.id,role_id:14},(res:any)=>{
        console.log("Is Member Of Class",res)
        if(res.length>0){
          this.isverifier = true;
        }
      })
      this.auth.isMemberOf({user_id:this.logindata.id,role_id:2},(res:any)=>{
        console.log("Is Member Of Class",res)
        if(res.length>0){
          this.isapprover1 = true;
        }
      })
      this.auth.isMemberOf({user_id:this.logindata.id,role_id:3},(res:any)=>{
        console.log("Is Member Of Class",res)
        if(res.length>0){
          this.isapprover2 = true;
        }
      })
      this.auth.isMemberOf({user_id:this.logindata.id,role_id:4},(res:any)=>{
        console.log("Is Member Of Class",res)
        if(res.length>0){
          this.isapprover3 = true;
        }
      })
      this.auth.isMemberOf({user_id:this.logindata.id,role_id:5},(res:any)=>{
        console.log("Is Member Of Class",res)
        if(res.length>0){
  ...
  submission_owner = function(_this:any,user_id:any){
    if(user_id===_this.logindata.id){
      return true;
    }else{
      return false;
    }
  ...
  editSubmission(submissionid:any){
    this.auth.isMemberOfDivision({user_id:this.logindata.id,division_id:1},(res:any) => {
      if(res[0].cnt>0){
    ...
    confirmer.afterClosed().subscribe(result=>{
      ...
      switch(result.answer){
        case 'yes':
          ...
          },(removedsubmissiondetail:any)=>{
            this.crud.doGet({
              url:'/movetotrash/submissions/'+obj.submission_id
            },(removedSubmission:any)=>{
  ...
  entryFixedPrice(submissionDetailId:any, submissionId:any){
  ...
  showSelectedStatus(){
    ...
    this.detailsToDisplay = this.detailsOrigin.filter((rows:any)=>{
  ...
  editVerification(submissionId:any){
    this.userService.checkRole({user_id:this.logindata.id,role_id:this._roles.verifier},(res:any) => {
      if(res.yes){
        this.router.navigate(['/verification/'+submissionId]);
      }
    })
  }
  detail(status:any, id:any, submissionId:any){
    ...
    switch(status){
      case 'Pending':
          ...
          this.userService.checkRole({user_id:this.logindata.id,role_id:this._roles.submitter},(res:any) => {
            if(res.yes){
      ...
      case 'Approved, bought and paid':
        this.userService.checkRole({user_id:this.logindata.id,role_id:this._roles.warehouse},(res:any) => {
          if(res.yes){
            this.router.navigate(['/goodReceived/'+submissionId])
          }
        })
        this.userService.checkRole({user_id:this.logindata.id,role_id:this._roles.purchase},(res:any) => {
          if(res.yes){
...
//        this.router.navigate(['/goodReceived/'+submissionId])
      ...
      case 'Received':
        ...
        this.submissionService.getSubmissionDetailBySubmissionId(submissionId,(res:any)=>{
...
console.log("DEtail",status,id,submissionId)
  ...
  reSendSubmissionMail(submission_id:any){
    this.submissionService.getSubmissionById(submission_id,(submissions:any)=>{
      let submission = submissions[0]
      console.log("Submission",submission)
      this.roleService.getRoleMail({identifier:'purchase'},(res:any)=>{
  ...
  rejectByVerifier(submission:any){
    const confirmer = this.dialog.open(CommonConfirmerComponent,{
      data:{
    ...
    confirmer.afterClosed().subscribe(result=>{
      ...
      switch(result.answer){
        case 'yes':
          this.rejectVerified(submission,(res:any)=>{
            this.reloadData()
  ...
  rejectVerified(obj:any,callback:any){
    this.crud.doGet({url:'/moverejectdetail/'+obj.submission_id},(res:any)=>{
